...
#Claudy = ctrl.Antecedent(np.arange(0, 2, 1), 'Claudy') #input7
...
x_train, x_test, y_train, y_test = train_test_split(x_vector, y_vector, test_size=1 - train_ratio , stratify=y_vector)

#Output csv to analyze data
...
y_fuzzy= np.zeros(len(y_test))
for i in range(len(y_test)):
        
    people_model.input['S1Light'] = x_test.iloc[i,0]
    people_model.input['S2Light'] = x_test.iloc[i,1]
    people_model.input['S3Light'] = x_test.iloc[i,2]
    people_model.input['DayTime'] = x_test.iloc[i,3]
...
print("Pecision:", precision_score(y_test, y_pred, average=None))
print("Recall:", recall_score(y_test, y_pred, average=None))
print("Macro-Pecision:", precision_score(y_test, y_pred, average='macro'))
